...
object PageRank extends Logging {
  def run(graph: Graph[Float, Long], numIter: Int, batchSize: Int,
          resetProb: Float = 0.15f): Graph[Float, Long] = {
    ...
    Pregel(graph, initialMsg = 0.0f, maxIterations = numIter,
      batchSize = batchSize, activeDirection = EdgeDirection.Both)(
      vprog, sendMsg, mergeMsg, active)

    graph
  }

  def sendMsg(ctx: EdgeTriplet[Float, Long]): Iterator[(VertexId, Float)] = {
    Iterator(
  ...
  def mergeMsg(m1: Float, m2: Float): Float = m1 + m2
}